...
if __name__ == '__main__':
    ...
    df = pandas.read_csv('./PN_analysis/final_distance_matrix.tsv', 
                         sep='\t', index_col=0)
    matrix = df.values
    ...
    for key, value in colors_taxonomy_mapping.items():
        ...
        #p = mpatches.Circle(xy=(0.5,0.5),color=value, label=key.capitalize(), radius=0.25)
    ...
    #c.clustergrid.savefig('./proteostasis_functional_analysis/sem_heatmap.tiff', 
    #                      format='tiff', dpi=800, pad_inches=0.02)
    c.clustergrid.savefig('./PN_analysis/PN_heatmap.png', 
                          format='png', dpi=800, pad_inches=0.02)
