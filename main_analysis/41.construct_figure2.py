...
def mds_worker(filename):
    ...
    colors = [colors_taxonomy_mapping[species_dict[i]] for i in df.index.tolist()]
...
if __name__ == '__main__':
    ...
    for taxonomy, tmp_df in taxonomy_groups:
        random_species = numpy.random.choice(tmp_df.species_name, 80)
    ...
    tmp_data = mds_coord_matrices['rRNA']
    worker_for_scatter_plot(ax, tmp_data['coords'], tmp_data['colors'],
    ...
    tmp_data = mds_coord_matrices['proteostasis_network']
    worker_for_scatter_plot(ax, tmp_data['coords'], tmp_data['colors'],
    ...
    tmp_data = mds_coord_matrices['hsp40']
    worker_for_scatter_plot(ax,tmp_data['coords'], tmp_data['colors'], 
    ...
    tmp_data = mds_coord_matrices['hsp70']
    worker_for_scatter_plot(ax, tmp_data['coords'], tmp_data['colors'], 
    ...
    for key, value in colors_taxonomy_mapping.items():
        patches[0].append(key.capitalize())
    ...
    for i, method in enumerate(['proteostasis_network', 'hsp40', 'hsp70']):
        ...
        sns.kdeplot(tmp_matrix[:,1], tmp_matrix[:,0], cmap=cmap1, n_levels=25, 
                    shade=True, bw=0.05, ax=contour_axes)
        ...
        sns.kdeplot(tmp_matrix[:,0], color="#08304a", shade=True, bw=0.025, 
        ...
        sns.kdeplot(tmp_matrix[:,1], color="#08304a", shade=True, bw=0.025, 
    ...
    plt.savefig('./comparison_of_phylogenetic_trees/figure_2.png', 
                dpi=600, format='png', bbox_inches='tight', pad_inches=0.05)

    plt.savefig('./comparison_of_phylogenetic_trees/figure_2.pdf', 
