...
if __name__ == "__main__":
    ...
    This task is executed only for bacteria, in order to get their taxonomic
    classification and then select manually at least one species from all the 
    Phyla. The reason is that there are thousands of bacterial reference 
    proteomes that pass the previous filtering step, so a smaller 
    representative set of them should be defined for the analysis.
    
    The script reads the 01_ranked_species_based_on_genomic_annotation.tsv 
    file to get the ranked bacteria with both GO and HSPs annotation and query
    for their taxonomies to UniProt.
    ...
    details_df.to_csv(main_dir+'files/bacteria_taxonomies.tsv', sep='\t')
